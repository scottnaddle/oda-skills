...
**4. [oda-country-research](oda-country-research/)** — 수원국 연구 (6 skills, 3 commands)

교육 프로파일, 니즈 평가, 이해관계자 매핑, 정책 환경, 디지털 준비도, 기초선 조사 설계. 6개국 references 포함.

| 커맨드 | 체이닝 | 설명 |
|---|---|---|
| `/research-country` | profile → policy → digital → needs | 국가 종합 연구 |
| `/map-stakeholders` | stakeholder-mapping (심화) | 이해관계자 심층 분석 |
| `/assess-needs` | needs-assessment → baseline-survey-design | 니즈 + 기초선 설계 |

**5. [oda-monitoring-evaluation](oda-monitoring-evaluation/)** — 모니터링 & 평가 (7 skills, 4 commands)

성과 프레임워크, 지표 설계, 중간/종료 평가, 영향 평가, 설문 분석, 데이터 품질.

| 커맨드 | 체이닝 | 설명 |
|---|---|---|
| `/setup-mne` | design-results-framework → define-indicators | M&E 체계 설계 |
| `/run-midterm` | midterm-evaluation → data-quality-check → survey-analysis | 중간평가 |
| `/run-endline` | endline-evaluation → impact-assessment → survey-analysis | 종료평가 |
| `/analyze-survey` | data-quality-check → survey-analysis | 설문 분석 |

**6. [oda-toolkit](oda-toolkit/)** — 유틸리티 (7 skills, 4 commands)

번역, 출장, 계약, 발표, 이메일, 통역, 교정.

| 커맨드 | 설명 |
|---|---|
| `/translate` | ODA 문서 번역 (한↔영) |
...
**7. [oda-partnership](oda-partnership/)** — 파트너십 & 협력 (6 skills, 3 commands)

발주처 전략, 현지 파트너 매핑, MOU/MOA, 하도급 범위, PSC 회의 준비, 수원국 정부 대응.

| 커맨드 | 체이닝 | 설명 |
|---|---|---|
| `/plan-partnership` | donor-strategy → local-partner → government-liaison | 파트너십 종합 전략 |
| `/draft-mou` | mou-draft (단독) | MOU/MOA 초안 |
| `/prepare-psc` | psc-preparation → progress-report* | PSC 회의 준비 |

**8. [oda-knowledge-management](oda-knowledge-management/)** — 지식관리 & 확산 (6 skills, 3 commands)

교훈, 성공사례, 사례연구, 성과확산, 후속사업, 역량이전.
...
| 스킬 | 53개 |
| 커맨드 | 30개 |
| 참조 파일 (references) | 24개 |
| 대응 발주처 | 5개 (KOICA, ADB, UNESCO, NIPA, NIA) |
| 국가별 프로파일 | 6개국 (KZ, VN, KH, LK, UZ, PH) |
